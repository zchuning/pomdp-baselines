...
class MetaDclawTurnEnv(DClawTurnEnv):
    def __init__(self, max_episode_steps, n_tasks):
        super().__init__()
        self._max_episode_steps = max_episode_steps
        self.n_tasks = n_tasks
        self.goals = [
            self.np_random.uniform(low=np.pi / 2, high=np.pi, size=(1,)) * (np.random.randint(2) * 2 - 1)
